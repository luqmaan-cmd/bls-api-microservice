...
def get_sa_data(
    ...
    data_type_code: Optional[str] = Query(None, description="Filter by data_type_code (comma-separated for OR)"),
    limit: int = Query(100, le=1000),
    offset: Optional[int] = Query(None, ge=0),
    page: Optional[int] = Query(None, ge=1),
    db: Session = Depends(get_db)
):
    query = db.query(SAData)
    ...
    if data_type_code:
        codes = parse_multi_value(data_type_code)
        filters["data_type_code"] = codes[0] if len(codes) == 1 else codes
    
    query = apply_filters(query, SAData, filters)
    ...
    query = apply_range_filters(query, SAData, range_filters)
